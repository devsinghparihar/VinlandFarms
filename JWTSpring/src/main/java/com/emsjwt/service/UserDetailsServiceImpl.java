...
//		Login user = this.load(username);
	...
	public Login loadFarmer(String email) {
		...
		Login userModel = new Login(user.getEmail(), user.getPassword(), user.getRole(), user.getFarmerId());
		return userModel;
	}

	public Login loadDealer(String email) {
		Dealer user = dealerClient.findDealerByEmail(email);
		Login userModel = new Login(user.getEmail(), user.getPassword(), user.getRole(), user.getDealerId());
		return userModel;
	}

	public Login loadAdmin(String email) {
		Admin user = adminClient.findByEmail(email);
		Login userModel = new Login(user.getEmail(), user.getPassword(), user.getRole(), user.getAdminId());
		return userModel;
	}
	...
	public String changePassword(ChangePasswordDTO changePasswordDTO) {
			...
			if(orignalOTP.isEmpty()) {
				...
				if(changePasswordDTO.getUserType().equals("Farmer")) {
				return farmerClient.changePassword(changePasswordDTO.getEmail(), changePasswordDTO.getNewPassword());
				}
				
				else if(changePasswordDTO.getUserType().equals("Dealer")) { return dealerClient.changePassword(changePasswordDTO.getEmail(), changePasswordDTO.getNewPassword());}
			}
			return "Illegal error occured";
	}
